...
void set_variable(const char *name, Value value) {
    ...
    if (index == -1) {
        ...
        variables[index].value = value;  // 更新变量的值
    }
}
...
Value eval_assignment(int l, int r) {
    ...
    for (int i = r; i >= l; i--) {
        if (tokens[i].type == TOKEN_OPERATOR) {
            if (tokens[i].str[0] == ')') parens++;
            else if (tokens[i].str[0] == '(') parens--;
        }
        if (parens == 0 && tokens[i].type == TOKEN_ASSIGNMENT) {
            assign_op = i;
            break;
        }
    }
    ...
    // 检查赋值语句的合法性
    if (assign_op == -1 || assign_op == l || assign_op == r) {
        Value error = { .type = VALUE_ERROR };
        return error;
    }

    // 检查赋值运算符左边是否是变量
    if (tokens[assign_op - 1].type != TOKEN_VARIABLE) {
        Value error = { .type = VALUE_ERROR };
        return error;
    }

    // 计算右边的表达式值
    Value val = eval(assign_op + 1, r);  // 改为直接使用 eval
    if (val.type == VALUE_ERROR) {
        return val;
    }

    // 先设置当前变量的值
    set_variable(tokens[assign_op - 1].str, val);

    // 如果左边还有赋值，递归处理
    if (assign_op > l) {
        return eval_assignment(l, assign_op - 1);
    }
